...
class FangindexSpider(CrawlSpider):
    ...
    start_urls = ['https://'+start_url]

    rules = (
        Rule(LinkExtractor(allow=r'http://zz\.'+host+'\.com/$'), callback='dircet_to_family', follow=True),     
    )
    ...
    def parse_item(self, response):
        item = {}
        # 调用处理城市首页的方法
        self.dircet_to_family(response)
